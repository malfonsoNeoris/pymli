...
from ..utils import get_object_config, huber_loss, mean_q


class SARSAAgent(Agent):
    def __init__(self, model, n_actions, policy=None, test_policy=None, gamma=.99, n_steps_warmup=10,
    ...
    def forward(self, observation):
        ...
        policy = self.policy if self.training else self.test_policy
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.n_steps_warmup and self.step % self.train_interval == 0:
            ...
            targets = np.zeros((1, self.n_actions))
            dummy_targets = np.zeros((1,))
            masks = np.zeros((1, self.n_actions))
